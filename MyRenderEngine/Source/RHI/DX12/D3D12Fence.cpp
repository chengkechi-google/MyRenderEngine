...
D3D12Fence::D3D12Fence(D3D12Device* pDevice, eastl::string name)
{
    m_pDevice = pDevice;
    m_name = name;
}
...
{
    ID3D12Device* pD3D12Device = (ID3D12Device*) m_pDevice->GetHandle();
    HRESULT result = pD3D12Device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_pFence));
    if (FAILED(result))
    {
        MY_ERROR("[D3D12Fence] failed to create {}", m_name);
        return false;
    }
