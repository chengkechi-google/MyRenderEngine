...
void GTAOPreFilterDepth16x16CS(uint2 dispatchThreadID : SV_DispatchThreadID, uint2 groupThreadID : SV_GroupThreadID)
{
    Texture2D<float> srcRawDepth = ResourceDescriptorHeap[c_srcRawDepth];
    SamplerState samplerPointClamp = SamplerDescriptorHeap[SceneCB.m_pointClampSampler];
    ...
    XeGTAO_PrefilterDepths16x16(dispatchThreadID, groupThreadID, gtaoCB, srcRawDepth, samplerPointClamp,
...
lpfloat3 LoadNormal(int2 pixCoord)
{
    Texture2D normalRT = ResourceDescriptorHeap[c_normalRT];
    float3 normal = DecodeNormal(normalRT.Load(int3(pixCoord, 0)).xyz);
    
    normal = mul(GetCameraCB().m_mtxView, float4(normal, 0.0)).xyz;
    return (lpfloat3) normal;
}

[numthreads(XE_GTAO_NUMTHREADS_X, XE_GTAO_NUMTHREADS_Y, 1)]
void GTAOMain(const uint2 pixCoord : SV_DispatchThreadID)
{
    ...
    if (srcWorkingDepth[pixCoord] == 0.0) //< No AO for infinite distance
    {
    ...
    XeGTAO_MainPass(pixCoord, sliceCount, stepsPerSlice, SpatioTemporalNoise(pixCoord, gtaoCB.NoiseIndex), LoadNormal(pixCoord),
        gtaoCB, srcWorkingDepth, samplerPointClamp, outWorkingAOTerm, outWorkingEdges);
}
...
[numthreads(XE_GTAO_NUMTHREADS_X, XE_GTAO_NUMTHREADS_Y, 1)]
void GTAODenoise(const uint2 dispatchThreadID : SV_DispatchThreadID)
{
