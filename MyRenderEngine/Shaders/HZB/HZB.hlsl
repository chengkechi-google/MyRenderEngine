...
cbuffer spdConstants : register(b1)
{
    ...
    // HLSL packing rules : every element in a array is store in a four-component vector
    uint4 c_imgDst[12]; 
}
...
AF4 SpdLoadSourceImage(ASU2 p, AU1 slice)
{
    ...
    SamplerState minSampler = SamplerDescriptorHeap[SceneCB.m_minReductionSampler];
    SamplerState maxSampler = SamplerDescriptorHeap[SceneCB.m_maxReductionSampler];
    AF2 textureCoord = p * c_invInputSize + (0.5 * c_invInputSize);
    
#if MIN_MAX_FILTER
    AF4 result = AF4(imgSrc.SampleLevel(minSampler, texureCoord, 0).x, imgSrc.SampleLevel(maxSampler, textureCoord, 0).y, 0, 0);
#else
...
{
    // May not need globallycoherent, because only call this function in the last group
    globallycoherent RWBuffer<uint> spdGlobalAutomic = ResourceDescriptorHeap[c_spdGlobalAtomicUAV];
    InterlockedAdd(spdGlobalAutomic[0], 1, spdCounter);
}
...
#define SPD_LINEAR_SAMPLER // We are using a min lenear reduction sampler
#include "../FFX/ffx_spd.h"

[numthreads(256, 1, 1)]
void BuildHZB(uint3 WorkGroupID : SV_GroupID, uint LocalThreadIndex : SV_GroupIndex)
{
    SpdDownsample(
        AU2(WorkGroupID.xy),
